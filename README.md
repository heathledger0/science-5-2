...
2. **활동 선택하기** — 확정된 핵심 개념으로 아래 4가지 활동 중 그날 쓸 것을 고릅니다.
   - 🕵️ **개념 일치 확인 게임**: 기존에 하시던 방식의 개선판. ① 모둠 수·탐색 시간·입력 시간을 설정하고
     ...
     학생이 직접 찾은 개념을 입력하고 **제출**합니다. 제출하면 입력이 잠기고, 그 화면에는 점수가 바로 뜨지
     않습니다 — 선생님이 그 태블릿에서 "선생님 확인용" 항목을 펼쳤을 때만 채점 결과가 나오는 방식이라,
     학생이 제출 버튼으로 정답을 미리 확인하는 걸 막았습니다. 오타로 보이는 답은 인정할지 그때 고를 수
     있습니다. 서버가 없어서 모둠별 점수가 교사 화면에 자동으로 모이지는 않고, 선생님이 각 모둠 태블릿을
     돌며 직접 확인합니다.
...
홈 화면의 **내보내기** 버튼으로 메모장으로도 바로 읽히는 `.txt` 파일을 받아 전달하고, 받은 선생님은
**파일로 불러오기**로 추가하면 됩니다.
...
js/unitTextFile.js      단원 내보내기/불러오기용 .txt 형식 변환
